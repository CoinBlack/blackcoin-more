...
FUZZ_TARGET(p2p_transport_serialization, .init = initialize_p2p_transport_serialization)
{
    ...
    V1Transport recv_transport{NodeId{0}, SER_NETWORK};
    V1Transport send_transport{NodeId{1}, SER_NETWORK};
...
{
    return std::make_unique<V1Transport>(nodeid, SER_NETWORK);
}
...
std::unique_ptr<Transport> MakeV2Transport(NodeId nodeid, bool initiator, RNG& rng, FuzzedDataProvider& provider)
{
    ...
    return std::make_unique<V2Transport>(nodeid, initiator, SER_NETWORK, key, ent, std::move(garb));
}
