...
uint64_t CWallet::GetStakeWeight() const
{
    ...
    {
        if (GetTxDepthInMainChain(*pcoin.first) >= Params().GetConsensus().nCoinbaseMaturity)
        {
...
bool CWallet::SelectCoinsForStaking(CAmount& nTargetValue, std::set<std::pair<const CWalletTx *, unsigned int> > &setCoinsRet, CAmount& nValueRet) const
{
    ...
    AvailableCoinsForStaking(*this, vCoins, &coincontrol);
...
bool CWallet::CreateCoinStake(ChainstateManager& chainman, const CWallet* pwallet, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, CAmount& nFees)
{
    ...
    while (true)
    {
        ...
        {
            SignatureData empty;
            if (!SignSignature(*pwallet->GetLegacyScriptPubKeyMan(), *pcoin.first->tx, txNew, nIn++, SIGHASH_ALL, empty))
