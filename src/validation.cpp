...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fProofOfStake, bool fOldClient)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!ContextualCheckBlockHeader(block, state, m_blockman, chainparams, pindexPrev, GetAdjustedTime(), chainstate.m_chain, fSetAsProofOfStake)) {
    ...
    CBlockIndex* pindex{m_blockman.AddToBlockIndex(block, fSetAsProofOfStake)};
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, bool fOldClient, const CBlockIndex** ppindex, const CBlockIndex** pindexFirst)
{
    AssertLockNotHeld(cs_main);
    {
        ...
        for (size_t i = 0; i < headers.size(); ++i) {
            ...
            bool accepted{AcceptBlockHeader(header, state, chainparams, &pindex, header.nFlags & CBlockIndex::BLOCK_PROOF_OF_STAKE, fOldClient)};
    ...
    if (NotifyHeaderTip(ActiveChainstate())) {
        if (ActiveChainstate().IsInitialBlockDownload() && ppindex && *ppindex) {
            ...
            const int64_t blocks_left{(GetTime() - last_accepted.GetBlockTime()) / chainparams.GetConsensus().nTargetSpacing};
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    bool accepted_header{m_chainman.AcceptBlockHeader(block, state, m_params, &pindex, block.IsProofOfStake())};
    CheckBlockIndex();
