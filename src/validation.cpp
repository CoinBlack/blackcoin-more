...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    std::string reason;
    if (m_pool.m_opts.require_standard && !IsStandardTx(tx, m_pool.m_opts.max_datacarrier_bytes, m_pool.m_opts.permit_bare_multisig, m_pool.m_opts.dust_relay_feerate, reason, witnessEnabled)) {
                                         ...
                                         PackageValidationState& package_state)
{
    ...
    if (const auto err_string{PaysForRBF(m_subpackage.m_conflicting_fees,
                                         m_subpackage.m_total_modified_fees,
                                         m_subpackage.m_total_vsize,
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, bool min_pow_checked, BlockValidationState& state, const CBlockIndex** ppindex,  const CBlockIndex** pindexFirst)
{
    ...
    if (NotifyHeaderTip()) {
        if (IsInitialBlockDownload() && ppindex && *ppindex) {
            ...
            int64_t blocks_left{(NodeClock::now() - last_accepted.Time()) / GetConsensus().TargetSpacing()};
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    if (initial_download) {
        int64_t blocks_left{(NodeClock::now() - NodeSeconds{std::chrono::seconds{timestamp}}) / GetConsensus().TargetSpacing()};
        blocks_left = std::max<int64_t>(0, blocks_left);
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        if (!CheckCanonicalBlockSignature(block)) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-signature-encoding", "AcceptBlock(): bad block signature encoding");
        }
