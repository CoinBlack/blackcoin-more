# Bootstrappable Blackcoin More Builds

This directory contains the files necessary to perform bootstrappable Blackcoin
More builds.
...
To build Blackcoin More reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Blackcoin More version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/blackcoin-more-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository, Guix build outputs are attested to in a `guix.sigs`
repository.
