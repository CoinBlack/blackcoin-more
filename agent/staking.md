...
12. **BIP152 compact block prefill fix (July 2)** — `blockencodings.cpp` constructor now prefills `vtx[1]` (coinstake) in addition to `vtx[0]` (marker) for PoS blocks. The coinstake is never in any peer's mempool, so short IDs always missed, forcing a `getblocktxn`→`blocktxn` round-trip on every PoS compact block. After the fix, PoS compact blocks are single-message (zero round-trips), matching how Bitcoin Core prefills the coinbase for PoW. Eliminates one round-trip per hop, reducing orphan rate. See `agent/CompactBlockFix.md`.
13. **Duplicate staking edge case with SignKey (July 3)** — Running the same wallet on multiple nodes creates a new orphan class not present with the old P2PK carrier. Previously, `vout[1]` carried a P2PK pubkey derived from the kernel UTXO's key — deterministic per UTXO. With the non-deterministic SignKey design, each wallet instance independently resolves its own `"SignKey"` entry at startup (`PoSMiner` → `GetNewDestination` if not found). Two instances racing at startup each generate a different SignKey, producing **different coinstakes** from the same kernel UTXO (different carrier pubkey, different DER-encoded scriptSig). The kernel proof is identical (same `prevout`, same `nTimeTx`), so both pass `CheckProofOfStake`.

    There are two failure modes depending on timing:
    - **Late loser:** The competing block connects before `ProcessBlockFound` runs (`miner.cpp:587`). `CheckProofOfStake` fails because the UTXO is already spent in `CoinsTip()`. The block is silently rejected with `LogError` — the wallet never sees it.
    - **Near-simultaneous:** Both blocks reach `ProcessNewBlock` on competing forks before the fork resolves. Both connect. When the chain reorganizes, one fork wins. The loser shows in `walletconflicts` with negative confirmations.

    Unlike the P2PK carrier era where duplicate instances produced identical coinstakes (same txid, detected by the network as duplicates), the non-deterministic SignKey design makes them different txs that race independently and create orphans. This only applies to **non-HD wallets or wallets that somehow have different SignKeys**; HD wallets sharing the same seed derive the same SignKey at `m/10'/0'/0'` and produce identical coinstakes (correctly rejected as duplicates). Warn users: **do not run the same wallet on multiple nodes simultaneously when staking** unless it is an HD wallet with the same seed.
