Building Blackcoin More with Visual Studio
==========================================

Introduction
---------------------
Visual Studio 2022 is minimum required to build Blackcoin More.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build Blackcoin More from the command-line, it is sufficient to only install the [Visual Studio Build Tools](https://visualstudio.microsoft.com/downloads/) component.
...
Qt
---------------------
To build Blackcoin More with the GUI, a static build of Qt is required.
...
To build Blackcoin More without Qt, unload or disable the `blackmore-qt`, `libblackmore_qt` and `test_blackmore-qt` projects.
...
```cmd
msbuild build_msvc\blackcoin.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/blackcoin.sln` file in Visual Studio.

Security
---------------------
[Base address randomization](https://learn.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization) is used to make Blackcoin More more secure. When building Blackcoin using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `blackmored` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/blackmored.exe
```
